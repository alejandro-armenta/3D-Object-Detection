...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            lbox = label.box
            lab_cor = tools.compute_box_corners(lbox.center_x, lbox.center_y, lbox.width, lbox.length, lbox.heading)
            ...
            for det in detections:
                ...
                det_cor = tools.compute_box_corners(x, y, w, l, yaw)

                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                
                dist = np.array([lbox.center_x, lbox.center_y, lbox.center_z]) - np.array([x,y,z])
                        

                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                l_poly = Polygon(lab_cor)
                ...
                if iou > min_iou:
                    matches_lab_det.append([iou, dist[0], dist[1], dist[2]])
    ...
    all_positives = len([v for v in labels_valid if v])
...
def compute_performance_stats(det_performance_all):
    ...
    all_positives_sum = np.sum([pn[0] for pn in pos_negs])
    true_positives_sum = np.sum([pn[1] for pn in pos_negs])
    false_negatives_sum = np.sum([pn[2] for pn in pos_negs])
    false_positives_sum = np.sum([pn[3] for pn in pos_negs])
